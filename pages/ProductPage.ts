...
export class ProductPage {
  ...
  async verifyFirstProductName(productName: string) {
    await expect(this.page.locator(this.productName).first()).toHaveText(
      productName,
    );
  }

  async verifyLastProductName(productName: string) {
    await expect(this.page.locator(this.productName).last()).toHaveText(
      productName,
    );
  }
  ...
  async verifyProductNameFromList(productName: string, listNumber: number) {
    ...
    await expect(
      this.page.locator(this.productName).nth(listNumber),
  ...
  async checkProductData(
    ...
    const currentProduct = this.page
      .locator(this.productCard)
      .filter({ hasText: productName });
    await expect(currentProduct.locator(this.productName)).toHaveText(
      productName,
    );
    await expect(currentProduct.locator(this.productDescription)).toHaveText(
      productDescription,
    );
    await expect(currentProduct.locator(this.productPrice)).toHaveText(
      productPrice,
    );
  }
  ...
  async accessProductPage(prouctName: string) {
    const currentProduct = this.page
      .locator(this.productCard)
      .filter({ hasText: prouctName });
    await currentProduct.locator(this.productName).click();
    await expect(this.page.locator(this.productPageName)).toHaveText(
      prouctName,
    );
  }
}
